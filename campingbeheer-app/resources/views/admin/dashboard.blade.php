        ...
        <div class="flex items-end gap-4">
            <div class="flex-1">
                ...
                <select id="admin-accommodatie-select"
                    ...
                    <option value="{{ $acc->id }}" data-titel="{{ $acc->titel }}">{{ $acc->titel }} ({{ $acc->type }})</option>
                    @endforeach
                </select>
            </div>
...
<script>
        ...
        searchInput.addEventListener('input', function () {
            ...
            debounceTimer = setTimeout(function () {
                fetch('/admin/zoek-gasten?q=' + encodeURIComponent(query))
                    ...
                    .then(function (data) {
                        if (data.length === 0) {
                            resultsContainer.innerHTML = '<div class="px-3 py-2 text-muted">Geen gasten gevonden</div>';
                        ...
                        data.forEach(function (gast) {
                            html += '<div class="gast-result px-3 py-2 cursor-pointer hover:bg-secondary border-b border-border last:border-0" data-naam="' + esc(gast.naam) + '" data-email="' + esc(gast.email) + '" data-telefoon="' + esc(gast.telefoon) + '" data-postcode="' + esc(gast.postcode) + '" data-huisnummer="' + esc(gast.huisnummer) + '" data-straat="' + esc(gast.straat) + '" data-plaats="' + esc(gast.plaats) + '" data-land="' + esc(gast.land) + '">';
                            html += '<div class="font-medium text-primary">' + esc(gast.naam) + '</div>';
                            if (gast.email) {
                                html += '<div class="text-xs text-muted">' + esc(gast.email) + '</div>';
                            }
                            html += '</div>';
                        });
    ...
    function fallbackFetchAddress(normalized, huisnummer) {
        ...
        function tryNominatim() {
            ...
            return fetch(url, {
                    headers: { 'User-Agent': 'Campingbeheer-App/1.0' }
                })
                .then(function (r) {
                    if (!r.ok) throw new Error();
                    return r.json();
