            ...
            <tbody class="divide-y divide-border">
                @forelse ($accommodaties as $acc)
                    <tr class="hover:bg-secondary/50 transition">
                        <td class="px-4 py-3 font-medium text-primary">{{ $acc->vertaaldeTitel($locale ?? 'nl') }}</td>
                        <td class="px-4 py-3 text-muted capitalize">{{ $acc->vertaaldType($locale ?? 'nl') }}</td>
                        ...
                        <td class="px-4 py-3 text-right">
                            ...
                            <form action="{{ route('admin.accommodatie.destroy', $acc) }}" method="POST" class="inline"
                                onsubmit="return confirm(window.__('admin.index.confirm_delete').replace('{name}', '{{ $acc->titel }}'))">
                                @csrf
                                @method('DELETE')
                ...
                @empty
                    <tr>
                        <td colspan="7" class="px-4 py-10 text-center text-muted">
                            ...
                            <a href="{{ route('admin.accommodatie.create') }}" class="text-accent hover:underline" data-i18n="admin.index.add_first">Voeg de eerste toe</a>.
                        </td>
                    </tr>
                @endforelse
            </tbody>
