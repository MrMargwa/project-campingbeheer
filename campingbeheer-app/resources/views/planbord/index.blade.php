...
@section('content')
    <section class="mx-auto max-w-7xl p-4">
            ...
            <table class="mx-auto w-full text-xs">
                <thead>
                    <tr class="border-b border-border bg-secondary/50">
                        <th
                            class="sticky left-0 z-10 min-w-[8rem] bg-secondary/50 px-2 py-1.5 text-left font-medium text-muted">
                            Accommodatie</th>
                        @foreach ($days as $day)
                            <th
                                class="min-w-[4rem] border-r border-border px-1 py-1.5 text-center font-medium last:border-r-0 {{ $day['isToday'] ? 'text-accent' : 'text-muted' }}">
                                <div class="text-[10px] uppercase leading-tight">{{ $day['label'] }}</div>
                                <div class="mt-0 leading-tight font-semibold">
                                    {{ \Carbon\Carbon::parse($day['date'])->format('d-m') }}</div>
                            </th>
                        @endforeach
                    </tr>
                </thead>
                    ...
                    @forelse($accommodaties as $accommodatie)
                        ...
                        <tr class="border-b border-border last:border-b-0">
                            ...
                            @foreach ($days as $day)
                                ...
                                @endphp
                                <td class="border-r border-border px-1 py-1 text-center align-middle last:border-r-0 {{ $cellClass }}"
        ...
        <div class="mt-3 flex flex-wrap items-center justify-center gap-x-4 gap-y-1 text-[10px] text-muted">
            ...
            <div class="flex items-center gap-1">
                <span class="inline-block h-3 w-3 rounded-sm"
                    style="background: linear-gradient(to bottom right, #f87171 50%, #86efac 50%);"></span>
                Check-out
            </div>
            <div class="flex items-center gap-1">
                <span class="inline-block h-3 w-3 rounded-sm"
