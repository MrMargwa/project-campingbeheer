...
<section class="mx-auto max-w-7xl p-6">
    <div class="mb-6 flex flex-wrap items-center justify-center gap-4">
        ...
        <a href="{{ route('admin.planbord.index', ['type' => $selectedType, 'week' => 0]) }}"
            class="rounded-lg border border-border bg-white px-4 py-2 text-sm font-medium text-primary transition hover:bg-secondary {{ $weekOffset === 0 ? 'ring-2 ring-accent/50' : '' }}">
            Deze week
        </a>
    </div>
        ...
        <table class="mx-auto w-full text-sm">
            <thead>
                <tr class="border-b border-border bg-secondary/50">
                    <th class="sticky left-0 z-10 min-w-[11rem] bg-secondary/50 px-4 py-3 text-left font-medium text-muted">Accommodatie</th>
                    @foreach($days as $day)
                ...
                @forelse($accommodaties as $accommodatie)
                    ...
                    <tr class="border-b border-border last:border-b-0 hover:bg-secondary/20">
                        ...
                        @foreach($days as $day)
                            ...
                            @endphp
                            <td class="px-3 py-3 text-center align-middle {{ $cellClass }}" title="{{ $title }}">{{ $label }}</td>
                        @endforeach
                    </tr>
                @empty
                    <tr>
                        <td colspan="8" class="px-4 py-12 text-center text-muted">Geen accommodaties gevonden.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
