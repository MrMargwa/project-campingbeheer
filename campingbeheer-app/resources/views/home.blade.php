...
@section('content')
    <section class="space-y-10">
        <div class="relative overflow-hidden rounded-3xl border border-border bg-surface shadow-sm">
            <div class="relative h-56 bg-secondary md:h-72 lg:h-80">
                <span class="absolute inset-x-0 top-1/2 h-px -rotate-8 bg-primary/80 origin-center"></span>
                <span class="absolute inset-x-0 top-1/2 h-px rotate-8 bg-primary/80 origin-center"></span>
            </div>
        ...
        <section id="results-wrapper" class="hidden space-y-5">
            ...
            <div id="no-results" class="hidden rounded-2xl border border-border bg-white px-4 py-3 text-sm text-muted">
                Geen chalets gevonden met deze filters.
            </div>
            ...
            <div id="results-list" class="space-y-6">
                ...
                <article class="chalet-card hidden overflow-hidden rounded-3xl border border-border bg-surface shadow-sm"
                    data-type="chalet" data-persons="4" data-features="wifi huisdieren">
                    <div class="grid lg:grid-cols-2">
                        <div class="relative min-h-64 bg-secondary sm:min-h-72 lg:min-h-full">
                            <span class="absolute left-4 top-4 h-px w-[85%] rotate-48 origin-top-left bg-primary/80"></span>
                            <span
                        ...
                        <div class="flex flex-col justify-between p-6 sm:p-8">
                            <div class="space-y-4">
                                <h3 class="text-2xl font-semibold text-primary">Chalet 2</h3>
                                <ul class="space-y-2 text-sm text-muted">
                                    <li>• Compleet ingerichte Limburgse chalets</li>
                                    <li>• 2 slaapkamers</li>
                                    <li>• Eigen douche, keuken en toilet</li>
    ...
    <script>
        (function () {
            ...
            function applyFilters() {
                ...
                const selectedPersons = personenSelect.value;
                ...
                cards.forEach((card, index) => {
                    const matchPersons = !selectedPersons || card.dataset.persons === selectedPersons;
            ...
            [personenSelect, typeSelect, kenmerkenSelect, aankomstInput, vertrekInput].forEach(element => {
                element.addEventListener('change', applyFilters);
            });

            aankomstInput.addEventListener('change', syncDateLimits);
            vertrekInput.addEventListener('change', syncDateLimits);
            syncDateLimits();
        })();
    </script>
@endsection