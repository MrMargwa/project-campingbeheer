...
class AccommodatieSeeder extends Seeder
{
    public function run(): void
    {
        ...
        foreach ($this->data() as $item) {
            ...
            $titel = str_replace('Camping', 'Camper', $item['title']);

            $basisNaam = strtolower(str_replace(' ', '-', $type));

            if (!isset($afbeeldingTellers[$type])) {
                $bestanden = glob(public_path('images') . '/' . $basisNaam . '-*.{jpg,jpeg,png,gif,webp}', GLOB_BRACE);
                $afbeeldingTellers[$type] = ['index' => 0, 'total' => count($bestanden)];
            }

            $teller = &$afbeeldingTellers[$type];

            if ($teller['total'] > 0) {
                $afbeeldingIndex = ($teller['index'] % $teller['total']) + 1;
                $afbeelding = $basisNaam . '-' . $afbeeldingIndex . '.png';
                $teller['index']++;
            } else {
                $afbeelding = $basisNaam . '.png';
            }
